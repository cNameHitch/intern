...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            cat <<EOF
Loop Commander uninstaller

Usage:
  uninstall.sh [OPTIONS]

Options:
  --data        Remove the ~/.loop-commander/ data directory without prompting.
...
PLIST_PATTERN="${LAUNCHAGENTS_DIR}/com.loopcommander.task.*.plist"
...
APP_BUNDLE="${APP_DIR}/Loop Commander.app"
...
SYSTEM_APP="/Applications/Loop Commander.app"
...
if [[ ${#removed[@]} -gt 0 ]]; then
    ...
    info "Nothing was removed — no Loop Commander files were found."
    printf "\n"
fi
...
if grep -qr "\.local/bin" "${HOME}/.zshrc" "${HOME}/.bash_profile" "${HOME}/.profile" 2>/dev/null; then
    ...
    warn "if it was added solely for Loop Commander."
fi
