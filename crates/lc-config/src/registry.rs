...
use chrono::Utc;
use lc_core::prompt::{AgentEntry, AgentRegistry};
use lc_core::LcError;
...
/// Manages the agent registry cache at `~/.loop-commander/agent-registry.json`.
...
impl RegistryManager {
    ...
    /// Return the built-in agent registry that ships with Loop Commander.
    fn builtin_registry() -> AgentRegistry {
        AgentRegistry {
            fetched_at: Utc::now(),
            agents: lc_core::builtin_agents::builtin_agents(),
        }
    }

    /// Save the registry to the cache file using atomic writes.
    pub fn save_cache(&self, registry: &AgentRegistry) -> Result<(), LcError> {
        let content = serde_json::to_string_pretty(registry)
            .map_err(|e| LcError::Config(format!("Failed to serialize registry: {e}")))?;
    ...
    pub async fn refresh(&self) -> Result<AgentRegistry, LcError> {
    ...
    pub fn ensure_prompts_dir(&self) -> Result<(), LcError> {
    ...
    pub fn save_prompt(&self, task_id: &str, content: &str) -> Result<PathBuf, LcError> {
    ...
    fn version_existing_prompt(&self, task_id: &str) -> Result<(), LcError> {
...
fn atomic_write_json(path: &Path, content: &[u8]) -> Result<(), LcError> {
...
fn atomic_write_md(path: &Path, content: &[u8]) -> Result<(), LcError> {
