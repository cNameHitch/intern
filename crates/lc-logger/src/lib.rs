//! SQLite persistence layer for Loop Commander execution logs.
//!
...
use lc_core::{DailyCost, DashboardMetrics, ExecStatus, ExecutionLog, LogQuery, Task, TaskMetrics};
...
/// (via `lc-runner`, which opens its own connection). The Swift app and CLI
...
impl Logger {
    ...
    pub fn get_dashboard_metrics(&self, tasks: &[Task]) -> Result<DashboardMetrics> {
        ...
        let active_tasks = tasks
            .iter()
            .filter(|t| t.status == lc_core::TaskStatus::Active)
            .count() as u64;
...
mod tests {
    ...
    fn sample_task(id: &str) -> Task {
        Task {
            id: lc_core::TaskId(id.to_string()),
            name: format!("Task {id}"),
            command: "echo hello".to_string(),
            skill: None,
            schedule: lc_core::Schedule::Interval { seconds: 300 },
