...
#[derive(Parser)]
#[command(name = "lc", about = "Loop Commander CLI", version = "0.1.0")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
async fn cmd_edit(
    ...
    let input = lc_core::UpdateTaskInput {
        id: id.clone(),
        name,
        command,
        skill: None,
...
async fn cmd_logs(
    ...
    for log in &logs {
        let status_icon = match log.status {
            lc_core::ExecStatus::Success => "  \u{2713}",
            lc_core::ExecStatus::Failed => "  \u{2715}",
            lc_core::ExecStatus::Timeout => "  T",
            lc_core::ExecStatus::Killed => "  K",
            lc_core::ExecStatus::Skipped => "  -",
        };
...
async fn cmd_export(id: String, output: Option<String>) -> anyhow::Result<()> {
    ...
    match output {
        Some(path) => {
            let expanded = lc_core::expand_tilde(&path);
...
async fn cmd_import(file: String, dry_run: bool) -> anyhow::Result<()> {
    let expanded = lc_core::expand_tilde(&file);
...
async fn cmd_daemon_install() -> anyhow::Result<()> {
    ...
    let plist_path = paths
        .launch_agents_dir
        .join("com.loopcommander.daemon.plist");
...
<dict>
    <key>Label</key>
    <string>com.loopcommander.daemon</string>
...
"#;
    ...
    println!();
    println!("Welcome to Loop Commander!");
    println!();
    println!("Get started:");
...
/// 2. `~/.cargo/bin/loop-commander`
/// 3. `/usr/local/bin/loop-commander`
/// 4. `which loop-commander`
fn find_daemon_binary() -> anyhow::Result<std::path::PathBuf> {
    // 1. Same directory as current exe
    if let Ok(current_exe) = std::env::current_exe() {
        if let Some(dir) = current_exe.parent() {
            let candidate = dir.join("loop-commander");
            if candidate.exists() {
                return Ok(candidate);
            }
        }
    ...
    if let Ok(home) = std::env::var("HOME") {
        let candidate = std::path::PathBuf::from(&home).join(".cargo/bin/loop-commander");
        if candidate.exists() {
            return Ok(candidate);
        }
    }
    ...
    // 3. /usr/local/bin
    let candidate = std::path::PathBuf::from("/usr/local/bin/loop-commander");
    if candidate.exists() {
        return Ok(candidate);
    }
...
mod tests {
    ...
    #[test]
    fn parse_list() {
        let cli = Cli::parse_from(["lc", "list"]);
    ...
    fn parse_rm_without_confirm() {
        let cli = Cli::parse_from(["lc", "rm", "lc-abc12345"]);
        match cli.command {
            Commands::Rm { id, y } => {
                assert_eq!(id, "lc-abc12345");
                assert!(!y);
    ...
    fn parse_rm_with_confirm() {
        let cli = Cli::parse_from(["lc", "rm", "lc-abc12345", "-y"]);
        match cli.command {
            Commands::Rm { id, y } => {
                assert_eq!(id, "lc-abc12345");
                assert!(y);
    ...
    #[test]
    fn parse_pause() {
        let cli = Cli::parse_from(["lc", "pause", "lc-abc12345"]);
        match cli.command {
    ...
    #[test]
    fn parse_resume() {
        let cli = Cli::parse_from(["lc", "resume", "lc-abc12345"]);
        match cli.command {
    ...
    #[test]
    fn parse_run() {
        let cli = Cli::parse_from(["lc", "run", "lc-abc12345"]);
        match cli.command {
    ...
    fn parse_run_dry_run() {
        let cli = Cli::parse_from(["lc", "run", "lc-abc12345", "--dry-run"]);
        match cli.command {
            Commands::Run { id, dry_run, json } => {
                assert_eq!(id, "lc-abc12345");
                assert!(dry_run);
    ...
    #[test]
    fn parse_run_json() {
        let cli = Cli::parse_from(["lc", "run", "lc-abc12345", "--dry-run", "--json"]);
        match cli.command {
            Commands::Run { id, dry_run, json } => {
                assert_eq!(id, "lc-abc12345");
                assert!(dry_run);
    ...
    #[test]
    fn parse_stop() {
        let cli = Cli::parse_from(["lc", "stop", "lc-abc12345"]);
        match cli.command {
    ...
    fn parse_logs_no_args() {
        let cli = Cli::parse_from(["lc", "logs"]);
        match cli.command {
            Commands::Logs {
                id,
                limit,
    ...
    #[test]
    fn parse_status() {
        let cli = Cli::parse_from(["lc", "status"]);
        assert!(matches!(cli.command, Commands::Status));
    }

    #[test]
    fn parse_export() {
        let cli = Cli::parse_from(["lc", "export", "lc-abc12345", "-o", "task.yaml"]);
        match cli.command {
    ...
    fn parse_export_stdout() {
        let cli = Cli::parse_from(["lc", "export", "lc-abc12345"]);
        match cli.command {
            Commands::Export { id, output } => {
                assert_eq!(id, "lc-abc12345");
    ...
    #[test]
    fn parse_import() {
        let cli = Cli::parse_from(["lc", "import", "task.yaml"]);
        match cli.command {
    ...
    fn parse_import_dry_run() {
        let cli = Cli::parse_from(["lc", "import", "task.yaml", "--dry-run"]);
        match cli.command {
            Commands::Import { file, dry_run } => {
                assert_eq!(file, "task.yaml");
                assert!(dry_run);
    ...
    fn parse_daemon_start() {
        let cli = Cli::parse_from(["lc", "daemon", "start"]);
        match cli.command {
    ...
    fn parse_daemon_stop() {
        let cli = Cli::parse_from(["lc", "daemon", "stop"]);
        match cli.command {
            Commands::Daemon { action } => {
    ...
    fn parse_daemon_status() {
        let cli = Cli::parse_from(["lc", "daemon", "status"]);
        match cli.command {
            Commands::Daemon { action } => {
    ...
    fn parse_daemon_install() {
        let cli = Cli::parse_from(["lc", "daemon", "install"]);
        match cli.command {
            Commands::Daemon { action } => {
    ...
    fn parse_config_get_all() {
        let cli = Cli::parse_from(["lc", "config", "get"]);
        match cli.command {
            Commands::Config { action } => match action {
    ...
    fn parse_config_get_key() {
        let cli = Cli::parse_from(["lc", "config", "get", "default_budget"]);
        match cli.command {
            Commands::Config { action } => match action {
    ...
    #[test]
    fn parse_config_set() {
        let cli = Cli::parse_from(["lc", "config", "set", "default_budget", "10.0"]);
        match cli.command {
            Commands::Config { action } => match action {
    ...
    #[test]
    fn parse_init() {
        let cli = Cli::parse_from(["lc", "init"]);
