//! JSON-RPC 2.0 dispatch for the Loop Commander daemon.
//!
...
async fn handle_task_run_now(
    ...
    // Discover the lc-runner binary path.
    ...
    // Spawn lc-runner as a child process.
    ...
    match child_result {
        ...
        Err(e) => {
            ...
            error!(
                task_id = %task_id,
                error = %e,
                "Failed to spawn lc-runner"
            );
            JsonRpcResponse::error(
                id,
                rpc_errors::SCHEDULER_ERROR,
                format!("Failed to spawn lc-runner: {e}"),
            )
        }
    }
}
...
async fn handle_task_dry_run(
    ...
    // Build resolved command using lc_runner's build_command.
    let resolved_command = build_command(&task);
...
async fn handle_schedule_validate(
    ...
    match lc_scheduler::validate_cron(&expression) {
...
mod tests {
    use super::*;
    use lc_core::{JsonRpcRequest, JsonRpcResponse};
