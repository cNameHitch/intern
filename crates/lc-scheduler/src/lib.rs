...
use lc_core::{LcPaths, Schedule, Task};
...
///
/// ```
/// assert!(lc_scheduler::validate_cron("0 9 * * 1-5").is_ok());
/// assert!(lc_scheduler::validate_cron("*/15 * * * *").is_ok());
/// assert!(lc_scheduler::validate_cron("not a cron").is_err());
/// ```
...
/// enum in `lc-core` only holds a single calendar entry, so this type captures
...
/// Manages launchd plists for Loop Commander tasks.
///
/// Each active task gets a plist at `~/.loop-commander/plists/<label>.plist`
...
impl Scheduler {
    /// Create a new scheduler, discovering the `lc-runner` binary on disk.
    ///
    /// # Errors
    ///
    /// Returns an error if the `lc-runner` binary cannot be found in any of the
    /// searched locations.
    pub fn new(paths: LcPaths) -> Result<Self> {
        let runner_path = find_runner().context("Failed to locate lc-runner binary")?;
        info!(?runner_path, "Discovered lc-runner binary");
        Ok(Self { paths, runner_path })
    }

    /// Return the path to the `lc-runner` binary that was discovered at
    /// construction time.
    #[must_use]
    ...
    pub fn deactivate(&self, task_id: &str) -> Result<()> {
        let label = format!("com.loopcommander.task.{task_id}");
        let uid = get_uid()?;
    ...
    fn build_plist(&self, task: &Task) -> plist::Dictionary {
        ...
        // EnvironmentVariables -- always include PATH so that lc-runner's child
...
/// Search for the `lc-runner` binary in standard locations.
///
/// Search order:
/// 1. Same directory as the current executable
/// 2. `~/.cargo/bin/lc-runner`
/// 3. `/usr/local/bin/lc-runner`
/// 4. `which lc-runner` (via `std::process::Command`)
fn find_runner() -> Result<PathBuf> {
    // 1. Same directory as current executable.
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            let candidate = dir.join("lc-runner");
            if candidate.is_file() {
                debug!(?candidate, "Found lc-runner alongside current executable");
                return Ok(candidate);
            }
        }
    }
    ...
    if let Some(home) = dirs::home_dir() {
        let candidate = home.join(".cargo/bin/lc-runner");
        if candidate.is_file() {
            debug!(?candidate, "Found lc-runner in ~/.cargo/bin/");
            return Ok(candidate);
        }
    }

    // 3. /usr/local/bin/lc-runner
    let candidate = PathBuf::from("/usr/local/bin/lc-runner");
    if candidate.is_file() {
        debug!(?candidate, "Found lc-runner in /usr/local/bin/");
        return Ok(candidate);
    }

    // 4. `which lc-runner`
    if let Ok(output) = Command::new("which").arg("lc-runner").output() {
        if output.status.success() {
            let path_str = String::from_utf8_lossy(&output.stdout).trim().to_string();
            if !path_str.is_empty() {
                let candidate = PathBuf::from(&path_str);
                if candidate.is_file() {
                    debug!(?candidate, "Found lc-runner via `which`");
                    return Ok(candidate);
                }
            }
        }
    ...
    anyhow::bail!(
        "Could not find lc-runner binary. Searched:\n  \
         1. Same directory as current executable\n  \
         2. ~/.cargo/bin/lc-runner\n  \
         3. /usr/local/bin/lc-runner\n  \
         4. `which lc-runner`\n\
         Please ensure lc-runner is built and available in one of these locations."
    )
}
...
mod tests {
    use super::*;
    use lc_core::{Schedule, Task, TaskId, TaskStatus};
    ...
    fn test_scheduler(dir: &std::path::Path) -> Scheduler {
        let paths = LcPaths::with_root(dir.to_path_buf());
        paths.ensure_dirs().expect("ensure_dirs");
        Scheduler {
            paths,
            runner_path: PathBuf::from("/usr/local/bin/lc-runner"),
        }
    }
    ...
    fn plist_generation_interval_contains_start_interval() {
        ...
        assert_eq!(
            ...
            Some("com.loopcommander.task.lc-testtest")
        );
        assert_eq!(
    ...
    fn plist_generation_includes_path_and_home() {
        ...
        // HOME so that lc-runner's child processes can find user-local binaries.
        let env = dict
            .get("EnvironmentVariables")
            .and_then(|v| v.as_dictionary())
            .expect("EnvironmentVariables should be present");
