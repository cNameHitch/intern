//! `lc-runner` -- standalone binary invoked by launchd for each task execution.
//!
...
use lc_core::{ExecStatus, ExecutionLog, LcPaths, TaskStatus};
...
/// CLI arguments for the lc-runner binary.
#[derive(Parser)]
#[command(name = "lc-runner", about = "Loop Commander task executor")]
struct CliArgs {
...
async fn main() -> Result<()> {
    ...
    info!(task_id = %task_id, "lc-runner starting");
    ...
    let command_argv = build_command(&task);
    ...
    info!(task_id = %task_id, exit_code = exit_code, "lc-runner finished");
...
async fn execute_task(
    ...
    let working_dir = if task.working_dir.to_string_lossy().starts_with('~') {
        lc_core::expand_tilde(&task.working_dir.to_string_lossy())
    } else {
...
fn restore_task_status(config_manager: &ConfigManager, task: &lc_core::Task, status: TaskStatus) {
