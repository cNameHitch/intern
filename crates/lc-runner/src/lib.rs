//! Shared logic for the `lc-runner` task executor.
//!
...
/// # Rules (CC-2)
///
/// - If `task.command` starts with `"claude"`: parse the command string into
///   argv tokens by splitting on whitespace while respecting single and double
///   quotes.
/// - If `task.command` does NOT start with `"claude"`: build
///   `["claude", "-p", &task.command, "--output-format", "json"]`.
...
#[must_use]
pub fn build_command(task: &Task) -> Vec<String> {
    let trimmed = task.command.trim();
    let mut argv = if trimmed.starts_with("claude") {
...
mod tests {
    ...
    async fn integration_echo_command_runs() {
        ...
        assert!(stdout.contains("hello from lc-runner test"));
    }
    ...
    fn check_budget_over_limit() {
        ...
        let log = lc_core::ExecutionLog {
            ...
            duration_secs: 60,
            exit_code: 0,
            status: lc_core::ExecStatus::Success,
            stdout: String::new(),
            stderr: String::new(),
            tokens_used: None,
            cost_usd: Some(50.0),
