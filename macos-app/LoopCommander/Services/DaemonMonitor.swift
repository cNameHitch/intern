...
class DaemonMonitor: ObservableObject {
    ...
    func startDaemon() async {
        ...
        let daemonPaths = [
            home.appendingPathComponent(".cargo/bin/loop-commander").path,
            "/usr/local/bin/loop-commander",
        ]
        ...
        if daemonPath == nil {
            let (whichStatus, whichData): (Int32, Data) = await Task.detached {
                ...
                process.arguments = ["loop-commander"]
                let pipe = Pipe()
        ...
        guard let path = daemonPath else {
            lastError = "Could not find loop-commander binary"
            return
        }
